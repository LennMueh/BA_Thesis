...
df_not_trained = df[(df.trained_between_iterations == False) & (df.epoch <= 5) & (df.initial_epochs == 1)].groupby('mutation_function')
df_trained = df[(df.trained_between_iterations == True) & (df.epoch <= 10) & (df.initial_epochs == 1)].groupby('mutation_function')
...
plt.savefig('figures/Mutatation_Trained_accuracy.png', bbox_inches='tight')
plt.show()
...
plt.grid()
plt.savefig('figures/Mutatation_NotTrained_accuracy.png', bbox_inches='tight')
plt.show()
...
plt.xticks(rotation=90)  # Adjust rotation as needed
plt.grid()
plt.savefig('figures/Mutatation_Trained_loss.png', bbox_inches='tight')
plt.show()
...
plt.xticks(rotation=90)  # Adjust rotation as needed
plt.grid()
plt.savefig('figures/Mutatation_NotTrained_loss.png', bbox_inches='tight')
plt.show()