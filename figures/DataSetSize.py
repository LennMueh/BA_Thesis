...
df_not_trained = df[(df.trained_between_iterations == False) & (df.epoch <= 5) & (df.initial_epochs == 1)].groupby('dataset')
df_trained = df[(df.trained_between_iterations == True) & (df.epoch <= 10) & (df.initial_epochs == 1)].groupby('dataset')
...
plt.grid()
plt.savefig('figures/Dataset_Trained_accuracy.png', bbox_inches='tight')
plt.show()
...
plt.xticks(rotation=45)  # Adjust rotation as needed
plt.grid()
plt.savefig('figures/Dataset_NotTrained_accuracy.png', bbox_inches='tight')
plt.show()
...
plt.xticks(rotation=45)  # Adjust rotation as needed
plt.grid()
plt.savefig('figures/Dataset_Trained_loss.png', bbox_inches='tight')
plt.show()
...
plt.xticks(rotation=45)  # Adjust rotation as needed
plt.grid()
plt.savefig('figures/Dataset_NotTrained_loss.png', bbox_inches='tight')
plt.show()